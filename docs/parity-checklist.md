...
8. ~~**Wiki の差分表示・リダイレクト・マクロエンジンが丸ごと未実装**。~~ → **一部done**(2026-07-21〜24)。差分表示・リダイレクトは既に完了(詳細は §Wiki 参照)。マクロエンジンは`{{toc}}`/`{{child_pages}}`を実装、`{{include}}`/`{{collapse}}`等は引き続き未実装(詳細は §マクロエンジン全体 参照)。
