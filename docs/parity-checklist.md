...
| 一括コピー・一括プロジェクト間移動・一括削除 | partial(2026-07-22) | 一括プロジェクト間移動(`move_issues`権限+移動先での`add_issues`、選択課題ごとに`IssueService::moveToProject()`)と一括削除(`delete_issues`権限、選択課題ごとに認可チェック後削除)を実装。一括コピーは未実装 |
