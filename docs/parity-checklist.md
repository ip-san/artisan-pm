...
1. ~~**プロジェクト横断ビューが皆無**(マイページを除く)。~~ → **一部done**(2026-07-22)。News(`news.global-index`、先行実装)に続き、Issues(`issues.global-index`)・TimeEntries(`time-entries.global-index`)・Search(`search.global-index`)のトップレベルビューを追加(詳細は §4/§Search 参照)。Activity(集計フィード自体はdone、プロジェクト単位限定)/Calendar/Gantt は引き続き `/projects/{project}/...` 配下限定のまま。
