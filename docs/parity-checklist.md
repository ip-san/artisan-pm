...
| プロジェクト(デフォルト公開設定、デフォルトモジュール、識別子連番化、新規プロジェクトの既定ロール) | partial(2026-07-23) | デフォルト公開設定(`default_projects_public`)・デフォルトモジュール(`default_projects_modules`)・デフォルトトラッカー(`default_projects_tracker_ids`)を実装(詳細は§2 Projects「プロジェクト作成」行を参照)。**識別子連番化を追加(2026-07-23)**: `sequential_project_identifiers`設定(Redmineの同名設定相当)。`Project::nextIdentifier()`が直近作成プロジェクト(id降順)の識別子を1つ進める — Redmine本体はRubyの汎用`String#succ`(末尾が数字なら数字側を繰り上げ、非数字ならアルファベット側を繰り上げてゼロ幅の桁上がりも行う)を使うが、本実装は「末尾の数字を+1(元の桁数を維持、桁上がり時のみ拡張)」のみ忠実に再現し、末尾が非数字の場合は単純に`1`を追記する簡略版(実用上ほぼ全てのケースをカバーし、稀なアルファベット桁上がりのみ非対応、という意図的な簡略化)。Redmine同様、新規プロジェクトフォームの初期表示では適用せず(`Project#set_default_values`と同じく保存時のみ、識別子欄が実際に空欄の場合のみ適用) — 本アプリの識別子欄は名前入力から自動スラグ化されるJS/Livewireの`updatedName()`が既にあるため、この設定が実際に効くのはユーザーが明示的に識別子欄を空にした場合のみ(Redmine自身も同じ制約を持つ、JSの自動生成を上書きしないための設計)。新規プロジェクトの既定ロールは引き続き未対応 |
| ユーザー | missing | — |
