...
9. ~~**Tracker・IssueStatus に管理画面が皆無**~~ → **done**(2026-07-21)。当初の監査では両方とも「done」と誤って報告されていたが、実際にはモデルのみでルート/画面が一切なく、さらにプロジェクト編集フォームにトラッカー選択欄が無いため UI 経由で作成したプロジェクトは課題を一切作成できない状態だった(トラッカーが0件のため)。この3点をまとめて修正。同じ調査で **ワークフロー遷移・フィールドルールの管理画面も同様に存在しない**ことが判明(ロジック・データモデルは実装済みで実際に使われているが、計画書で明記されていたトラッカー×ロールの遷移グリッド編集画面自体が無い)— これは未修正、§1 参照。
...
| **ワークフロー遷移・フィールドルールの管理画面** | **missing** | `WorkflowService`/`workflow_transitions`/`workflow_field_rules` はロジック・データモデルとも実装済みで実際に使われているが、計画書で明記されていた「トラッカー×ロールを選んでからステータス×ステータスの遷移グリッドを編集するLivewireコンポーネント」自体が存在しない。現状は遷移ルールをseeder/tinker以外で設定する手段がなく、Tracker/IssueStatusと同種の「モデルはあるが管理画面がない」ギャップ |
