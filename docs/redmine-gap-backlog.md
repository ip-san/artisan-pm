...
| 28a | A11-17 | A11-01, A1-17 | M | blocked(依存: A1-17) |
...
| 99b | A1-10b | A1-10 | S〜M | todo |
| 99c | A1-10c | A1-10 | S | todo |
...
| 132 | A12-03 | — | M | blocked(要承認: docs/design/gap-A12-03.md) |
...
| A14-01b | 画面ごとの文字列を `__('原文')` で包み、`lang/en.json` に英訳を足す(課題 → プロジェクト → 工数 → Wiki → 管理画面、続いてメール、PDF/CSV、enum の `label()`) | 未着手 | 原文がキーの JSON 方式 | A14-01a | M×n | 「多言語」 |
...
| 行 | 内容 | 設計メモ | 判断点 |
|---|---|---|---|
| A1-10 | カスタムフィールド形式 `user`/`version` | [gap-A1-10](design/gap-A1-10.md) | 保存形式、既存の値の扱い、可視性 |
...
| A12-06c | メールからのアカウント作成(`unknown_user` など) | [gap-A12-06c](design/gap-A12-06c.md) | 実装しない案を推奨。`no_notification` だけ別行 |
| A14-01 | 多言語対応(L) | [gap-A14-01](design/gap-A14-01.md) | `__()` の段階導入(推奨) |
| A5-16b | 開始日の既定を API/受信メールにも適用 | (行内) | 既定がオンのため既存クライアントの挙動が変わる |
...
- **フォローアップ行(実装済み)**: A1-05b、A1-06b、A2-07b、A8-02b、A9-03b、A9-04b、A12-02b、A12-06b、A13-05b。
- **フォローアップ行(承認待ち)**: A12-06c(メモ参照。実装しない案の場合の残りは A12-06d `no_notification`)。
- **バックログ自身の訂正(§C)**: C-24〜C-28、C-30(Redmine 7.0 に無い、または既に実装済みだった主張)。
- **実装中に見つけた不具合(修正済み)**: C-29(CSV エクスポートの数式インジェクション、`CsvCell` で対策)ほか。
