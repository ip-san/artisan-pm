    ...
    <form wire:submit="save" class="space-y-4">
        ...
        <div>
            ...
            @else
                ...
                @error('field_format') <p class="mt-1 text-sm text-danger-bolder">{{ $message }}</p> @enderror
            @endif
        </div>
                ...
                <div class="flex flex-wrap gap-3">
                    @foreach ($this->roles as $role)
                        <label class="flex items-center gap-2 text-sm text-neutral-700">
                            <input type="checkbox" wire:model="userRoleIds" value="{{ $role->id }}" class="rounded border-neutral-300">
                            {{ $role->name }}
                        </label>
                    @endforeach
                </div>
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-3">
                    @foreach (\App\Enums\VersionStatus::cases() as $status)
                        <label class="flex items-center gap-2 text-sm text-neutral-700">
                            <input type="checkbox" wire:model="versionStatuses" value="{{ $status->value }}" class="rounded border-neutral-300">
                            {{ $status->value }}
                        </label>
                    @endforeach
                </div>
        ...
        @if ($field_format === \App\Enums\CustomFieldFormat::Progressbar->value)
            <div>
                ...
                @error('ratio_interval') <p class="mt-1 text-sm text-danger-bolder">{{ $message }}</p> @enderror
            </div>
        @endif
        ...
        <div>
            ...
            @error('default_value_mode') <p class="mt-1 text-sm text-danger-bolder">{{ $message }}</p> @enderror
        </div>
        ...
        <div>
            ...
            <div class="flex flex-wrap gap-3">
                @foreach ($this->projects as $project)
                    <label class="flex items-center gap-2 text-sm text-neutral-700">
                        <input type="checkbox" wire:model="projectIds" value="{{ $project->id }}" class="rounded border-neutral-300">
                        {{ $project->name }}
                    </label>
                @endforeach
            </div>
