...
<div>
    <h1 class="text-xl font-semibold text-neutral-900 mb-6">{{ $project->name }} — 検索</h1>
    ...
    <form wire:submit="search" class="mb-6 space-y-3">
        ...
        <div class="flex flex-wrap gap-4 text-sm text-neutral-700">
            ...
            <label class="flex items-center gap-1.5">
                <input type="checkbox" wire:model="includeSubprojects" class="rounded border-neutral-300">
                サブプロジェクトを含む
            </label>
        </div>
    </form>
        ...
        <ul class="space-y-3">
            @foreach ($this->pagedResults as $result)
                <li wire:key="result-{{ $result->type }}-{{ $result->url }}" class="rounded-md border border-neutral-200 bg-white p-4">
                    ...
                    @endif
                    <p class="mt-1 text-xs text-neutral-400">{{ $result->updatedAt->format('Y-m-d H:i') }}</p>
                </li>
            @endforeach
        </ul>
