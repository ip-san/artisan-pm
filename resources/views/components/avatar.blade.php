...
@if ($user !== null)
    <span {{ $attributes->merge(['class' => 'relative inline-block shrink-0 align-middle']) }} style="width: {{ $px }}px; height: {{ $px }}px" data-avatar title="{{ $name }}">
        @if ($fallback)
        ...
        @endif
        @if ($showGravatar)
            <img src="{{ \App\Support\Avatar\UserAvatar::gravatarUrl($user->email, $px) }}" alt="{{ $name }}" width="{{ $px }}" height="{{ $px }}"
