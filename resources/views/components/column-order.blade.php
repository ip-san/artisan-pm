...
@if (count($columns) > 1)
    <div class="flex flex-wrap items-center gap-2 text-sm text-neutral-700" data-column-order>
        ...
        @foreach ($columns as $index => $key)
            <span class="flex items-center gap-1 rounded border border-neutral-200 bg-white px-1.5 py-0.5" wire:key="column-order-{{ $key }}">
                {{ $labels[$key] ?? $key }}
                <button type="button" wire:click="moveColumn('{{ $key }}', -1)" @disabled($index === 0)
                    class="text-neutral-500 hover:text-neutral-900 disabled:opacity-30" title="前へ">◀</button>
                <button type="button" wire:click="moveColumn('{{ $key }}', 1)" @disabled($loop->last)
                    class="text-neutral-500 hover:text-neutral-900 disabled:opacity-30" title="後ろへ">▶</button>
            </span>
        @endforeach
    </div>
@endif
