...
final class MessagePolicy
{
    ...
    public function viewWatchers(?User $user, Message $message): bool
    {
        return $message->isTopic() && $this->authorization->can($user, 'view_message_watchers', $message->board->project);
    }

    /**
     * Adding *other* users as watchers — distinct from watch(), which lets
     * anyone with view access toggle their own watch state. Topics only.
     */
    public function addWatchers(User $user, Message $message): bool
    {
        return $message->isTopic() && $this->authorization->can($user, 'add_message_watchers', $message->board->project);
    }

    public function deleteWatchers(User $user, Message $message): bool
    {
        return $message->isTopic() && $this->authorization->can($user, 'delete_message_watchers', $message->board->project);
    }
}
